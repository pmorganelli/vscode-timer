...
function exportAsCSV() {
        ...
        let csvContent = "File Name, Active Time (ms), Idle Time (ms), Total Time (ms)\n"; // CSV headers
        let totalTime = 0;
        let totalActive = 0;
        let totalIdle = 0;
        for (let curr of fileData) {
                let currTotal = curr.activeTime + curr.idleTime;
                csvContent += `${curr.name},${curr.activeTime},${curr.idleTime},${currTotal}\n`;
                totalActive += curr.activeTime;
                totalIdle += curr.idleTime;
                totalTime += curr.activeTime + curr.idleTime;
        }
        csvContent += `Totals: ,${totalActive},${totalIdle},${totalTime}`
